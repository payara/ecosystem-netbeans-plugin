...
import org.netbeans.modules.payara.common.SimpleIO;
import java.io.File;
...
/** 
 * This action will run the update center tool for Payara.
 * 
...
public class ViewUpdateCenterAction extends NodeAction {
    ...
    private static final String SHOW_UPDATE_CENTER_ICONBASE = 
            "org/netbeans/modules/payara/common/resources/UpdateCenter.gif"; // NOI18N
    ...
    protected void performAction(Node[] nodes) {
        ...
        final PayaraModule commonSupport = nodes[0].getLookup().lookup(PayaraModule.class);
        if(commonSupport != null) {
            ...
            final String serverUrl = ip.get(PayaraModule.URL_ATTR);
            final String serverName = ip.get(PayaraModule.DISPLAY_NAME_ATTR);
            ...
            final String installRoot = ip.get(PayaraModule.INSTALL_FOLDER_ATTR);
                ...
                RequestProcessor.getDefault().post(new Runnable() {
                    public void run() {
                        try {
                            ...
                            String path = commonSupport.getInstanceProperties().get(PayaraModule.PAYARA_FOLDER_ATTR);
                            ...
                            Logger.getLogger("payara").log(Level.WARNING, ex.getLocalizedMessage(), ex);
                        }
                    }
                });
            } else {
    ...
    /**
     * Locate update center launcher within the payara installation
    ...
    private boolean installUpdateCenter(String serverName, String serverUrl, File installRoot, File launcher) {
        ...
        if(DialogDisplayer.getDefault().notify(nd) == NotifyDescriptor.YES_OPTION) {
            ...
            try {
                ...
                Logger.getLogger("payara").log(Level.FINEST, "UC exit code = " + exitCode);
                if(exitCode == 0) {
                    writer.close();
                    writer = null;
                    ucIO.closeIO();
    ...
    protected boolean enable(Node[] nodes) {
        if(nodes != null && nodes.length == 1 && nodes[0] != null) {
            ...
            if(commonSupport != null) {
                String installRoot = commonSupport.getInstanceProperties().get(PayaraModule.INSTALL_FOLDER_ATTR);
    ...
    private void displayAdminWarning() {
        ...
        Logger.getLogger("payara").warning(mess);
