...
public class JavaEEServerModuleFactory implements PayaraModuleFactory {
    ...
    public Object createModule(Lookup instanceLookup) {
        ...
        if(commonModule != null) {
            ...
            if (ip == null) {
                ...
                String displayName = props.get(InstanceProperties.DISPLAY_NAME_ATTR);
                    try {
                        ...
                        ip = InstanceProperties.getInstanceProperties(url);
                        if (null == ip) {
                            Logger.getLogger("payara-javaee").log(Level.WARNING, null, ex); // NOI18N
                        }
                    }

                if(ip == null) {
                    Logger.getLogger("payara-javaee").log(Level.INFO, "Unable to create/locate J2EE InstanceProperties for {0}", url);
                }
            }
            ...
            Logger.getLogger("payara-javaee").log(Level.WARNING, "commonModule is NULL");
        }
    ...
    private static boolean ensureEclipseLinkSupport(String installRoot) {
        ...
        try {
            ...
            if (j2eeDoc != null) {
                ...
                Logger.getLogger("payara-javaee").log(Level.WARNING, "Warning: Java EE documentation not found when registering EclipseLink library.");
            }
        } catch (MalformedURLException ex) {
            Logger.getLogger("payara-javaee").log(Level.WARNING, ex.getLocalizedMessage(), ex);
            return false;
        }
    ...
    private static final String JERSEY_PF_SERVER = "jersey-gf-server"; //NOI18N
    ...
    private static final String[] JAXRS_LIBRARIES_31 =
             {"jackson-core-asl", "jackson-jaxrs", "jackson-mapper-asl", "jersey-client", "jersey-core", JERSEY_PF_SERVER, "jersey-json", "jersey-multipart", "jettison", "mimepull"}; //NOI18N
    ...
    private static boolean ensurePayaraApiSupport(PayaraServer server) {
        ...
        if (j2eeDoc != null) {
            try {
                ...
                Logger.getLogger("payara-javaee").log(Level.INFO, "Problem while registering Java EE API library JavaDoc."); // NOI18N
            }
        } else {
            Logger.getLogger("payara-javaee").log(Level.INFO, "Java EE documentation not found when registering Java EE API library."); // NOI18N
        }
        ...
        if (f != null && f.exists()) {
            try {
                ...
                Logger.getLogger("payara-javaee").log(Level.INFO, "Problem while registering web-core into Payara API library."); // NOI18N
            }
        }

        File jerseyGFServer = ServerUtilities.getJarName(installRoot, JERSEY_PF_SERVER + ServerUtilities.GFV3_VERSION_MATCHER);
        ...
        if (lib != null) {
            ...
            for (URL libUrl : libList) {
                ...
                if (!new File(libPath.replace("!/", "")).exists()) {
                    Logger.getLogger("payara-javaee").log(Level.FINE, "libPath does not exist.  Updating {0}", name);
                    try {
                        lmgr.removeLibrary(lib);
                    } catch (IOException ex) {
                        Logger.getLogger("payara-javaee").log(Level.INFO, ex.getLocalizedMessage(), ex);
        ...
        if (lib == null) {
            ...
            try {
                ...
                if (null != ltp) {
                    ...
                    Logger.getLogger("payara-javaee").log(Level.FINE, "Created library {0}", name);
                } else {
                    lmgr.addPropertyChangeListener(new InitializeLibrary(lmgr, libType, name, contents, displayName, description));
                    Logger.getLogger("payara-javaee").log(Level.FINE, "schedule to create library {0}", name);
                }
            } catch (IOException ex) {
                ...
                if (null != name) {
                    ...
                    if (null == l) {
                        try {
                            ...
                            Logger.getLogger("payara-javaee").log(Level.INFO,
